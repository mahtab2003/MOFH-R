		...
		<div class="row">
			<div class="col-md-6">
				<div class="card mb-2">
					...
					<div class="card-body">
						<?= form_open('n/view_client/'.$data['key'].'?general=true') ?>
							<div class="mb-2">
								<label class="form-label"><?= $this->ui->text('name_text') ?></label>
								<input type="text" name="name" class="form-control" placeholder="<?= $this->ui->text('name_placeholder_text') ?>" value="<?= $data['name'] ?>" required>
							</div>
							<div class="mb-3">
								<label class="form-label"><?= $this->ui->text('email_text') ?></label>
								<input type="email" class="form-control" placeholder="<?= $this->ui->text('email_placeholder_text') ?>" value="<?= $data['email'] ?>" readonly>
							</div>
							<div class="mb-0">
			...
			<div class="col-md-6">
				<div class="card mb-2">
					...
					<div class="card-body">
						<?= form_open('n/view_client/'.$data['key'].'?pref=true') ?>
							<div class="mb-2">
								<label class="form-label"><?= $this->ui->text('status_text') ?></label>
								<select class="form-control" name="status">
									<?php foreach (['active', 'inactive'] as $status): ?>
										<option value="<?= $status ?>" <?php if($status == $data['status']): echo('selected'); endif; ?>><?= ucfirst($status) ?></option>
									<?php endforeach ?>
								</select>
							</div>
							<div class="mb-3">
								<label class="form-label"><?= $this->ui->text('role_text') ?></label>
								<select class="form-control" name="role">
									...
									<?php foreach ($roles as $role): ?>
										<option value="<?= $role ?>" <?php if($role == $data['role']): echo('selected'); endif; ?>><?= ucfirst($role) ?></option>
									<?php endforeach ?>
								</select>
							</div>
							<div class="mb-0">
			...
			<div class="col-md-6">
				<div class="card mb-2">
					...
					<div class="card-body">
						<?= form_open('n/view_client/'.$data['key'].'?security=true') ?>
							<div class="mb-2">
			...
			<div class="col-md-6">
				<div class="card mb-2">
					...
					<div class="card-body">
						<?= form_open('n/view_client/'.$data['key'].'?2fa=true') ?>
							<div class="mb-2">
								<label class="form-label"><?= $this->ui->text('2fa_text') ?></label>
								<input type="text" name="2fa" class="form-control" placeholder="<?= $this->ui->text('password_placeholder_text') ?>" value="<?= $data['2fa_key'] ?>" readonly>
							</div>
							<div class="mb-3">
								<label class="form-label"><?= $this->ui->text('status_text') ?></label>
								<select class="form-control" name="status">
									<?php foreach (['active', 'inactive'] as $status): ?>
										<option value="<?= $status ?>" <?php if($status == $data['2fa_status']): echo('selected'); endif; ?>><?= ucfirst($status) ?></option>
									<?php endforeach ?>
								</select>
							</div>
							<div class="mb-0">
