		...
		<div class="row">
			...
			<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right">
					...
					<li class="breadcrumb-item"><a href="<?= base_url('t/all_ticket') ?>"><?= $this->ui->text('support_link_text') ?></a></li>
	...
	<div class="container-xl">
		<div class="card mb-2">
			<div class="card-header">
				<h3 class="card-title"><?= $this->ui->text('all_ticket_text') ?></h3>
			</div>
				...
				<table class="table">
					<thead>
						<tr>
							...
							<th width="20%"><?= $this->ui->text('subject_tab_text') ?></th>
							<th width="40%"><?= $this->ui->text('email_tab_text') ?></th>
							<th width="15%"><?= $this->ui->text('date_tab_text') ?></th>
						...
						<?php if (is_array($list) AND count($list) > 0): ?>
							...
							<?php for($i = $start; $i < $end; $i++){ ?>
								<tr>
									...
									<td class="text-truncate"><?= $this->user->get([], ['key' => $list[$i]['for']])[0]['email'] ?></td>
									</td>
									<td class="text-truncate"><?= date('d.m.Y', $list[$i]['date']) ?></td>
									...
									<td class="text-truncate">
										<a href="<?= base_url('t/view_ticket/'.$list[$i]['key']) ?>" class="btn btn-primary btn-sm"><?= $this->ui->text('manage_btn_text') ?></a>
									</td>
								</tr>
							<?php } ?>
						<?php else: ?>
