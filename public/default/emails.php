	...
	<div class="container-xl">
		<div class="card mb-2">
			<div class="card-header">
				<h3 class="card-title"><?= $this->ui->text('emails_text') ?></h3>
			</div>
				...
				<table class="table">
					<thead>
						<tr>
							...
							<th width="75%"><?= $this->ui->text('subject_tab_text') ?></th>
							<th width="10%"><?= $this->ui->text('trigger_tab_text') ?></th>
						...
						<?php if (is_array($list) AND count($list) > 0): ?>
							...
							<?php for($i = $start; $i < $end; $i++){ ?>
								<tr>
									...
									<td class="text-truncate"><?= $list[$i]['subject'] ?></td>
									<td class="text-truncate"><?= strtoupper($list[$i]['id']) ?></td>
									<td class="text-truncate">
										<a href="<?= base_url('n/edit_email/'.$list[$i]['id']) ?>" class="btn btn-primary btn-sm"><?= $this->ui->text('manage_btn_text') ?></a>
									</td>
								</tr>
							<?php } ?>
						<?php else: ?>
			...
			<div class="card-footer">
				<div class="d-flex justify-content-between align-items-center">
					<div><?= $this->ui->text('showing_text') ?> <?= $start + 1 ?? 0 ?> <?= $this->ui->text('to_text') ?> <?= $end ?? 0 ?> <?= $this->ui->text('of_text') ?> <?= count($list) ?> <?= $this->ui->text('entites_text') ?></div>
					<ul class="pagination pagination-sm float-right m-0">
						...
						if ($page > 0):
							$page1 = $page - 1;
							$link1 = base_url('n/emails?page='.$page1);
							$status1 = '';
						else:
							$link1 = '#';
							$status1 = 'disabled';
						...
						if($page < $i):
							$page2 = $page + 1;
							$link2 = base_url('n/emails?page='.$page2);
							$status2 = '';
						else:
							$link2 = '#';
							$status2 = 'disabled';
