...
class Hosting extends CI_Model
{
	...
	function create($label, $domain)
	{
		...
		$username = hash_64($label.':'.$domain.':'.$user['email'].':'.time());
		$password = substr(hash_128($username.':'.time()), 0, 15);
	...
	function suspend($username, $reason)
	{
		...
		elseif($res->isSuccessful() == 0 AND !is_array($res->getMessage()) > 1)
		{
			return trim($res->getMessage());
		}
		elseif($res->isSuccessful() == 1 AND is_array($res->getMessage()) > 1)
		{
